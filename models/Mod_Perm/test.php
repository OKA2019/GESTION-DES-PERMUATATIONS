<?php
    ...
    function PermBiepNT()
    {
        ...
        (SELECT nom_iep FROM iep,dossier_deman WHERE iep.Id_iep = dossier_deman.Id_iep) AS dem_iep,
        (SELECT nom_dren FROM dren ,dossier_deman WHERE dren.Id_dren = dossier_deman.Id_dren) AS dem_dren,
        (SELECT nom FROM personnel, dossier_deman WHERE personnel.matricule = dossier_deman.matricule) AS dem_nomC,
        (SELECT prenoms FROM personnel, dossier_deman WHERE personnel.matricule = dossier_deman.matricule) AS dem_prenomC
        FROM personnel
        JOIN enseig_favorable ON enseig_favorable.annee = '$statutAN[0]'
        JOIN dossier_fav ON (enseig_favorable.matricule = dossier_fav.matricule)
        JOIN dossier ON ( (dossier_fav.id_dossier = dossier.id_dossier) AND (statut_iep2 IS NULL))
        WHERE dossier_fav.Id_iep = '$mat_iep' ");
    ...
    function PermBiepT()
    {
        ...
        // Faire une jointure pour renvoyer les dossiersv non traiter
        $PermBiepT = $user->query("SELECT DISTINCT nom, prenoms, telephone, statut_iep2,
        (SELECT nom_iep FROM iep,dossier_deman WHERE iep.Id_iep = dossier_deman.Id_iep) AS dem_iep,
        (SELECT nom_dren FROM dren,dossier_deman WHERE dren.Id_dren = dossier_deman.Id_dren) AS dem_dren,
        (SELECT nom FROM personnel, dossier_deman WHERE personnel.matricule = dossier_deman.matricule) AS dem_nomC,
        (SELECT prenoms FROM personnel, dossier_deman WHERE personnel.matricule = dossier_deman.matricule) AS dem_prenomC
